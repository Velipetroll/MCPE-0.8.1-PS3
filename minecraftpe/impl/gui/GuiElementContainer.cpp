...
bool_t GuiElementContainer::backPressed(Minecraft* a2, bool_t a3) {
	bool_t ret = 0;
	for(auto& i: this->children) {
		if(i && i->backPressed(a2, a3)) ret = 1;
	}
	return ret;
}
...
bool_t GuiElementContainer::suppressOtherGUI() {
	for(auto& i: this->children) {
		if(i && i->suppressOtherGUI()) return 1;
	}
	return 0;
}
