...
void Minecraft::_levelGenerated(void) {
	...
	NetEventCallback* serverSideNetworkHandler;
...
void Minecraft::audioEngineOff(void) { this->soundEngine->enable(0); }
void Minecraft::audioEngineOn(void) { this->soundEngine->enable(1); }
...
void Minecraft::gameLostFocus(void) {
	if(this->currentScreen) this->currentScreen->lostFocus();
}
...
int32_t Minecraft::getCreator(void) { return 0; }
struct LevelStorageSource* Minecraft::getLevelSource(void) { return this->levelStorageSource; }
int32_t Minecraft::getLicenseId(void) {
	if(this->license < 0) this->license = AppPlatform::_singleton->checkLicense();
	return this->license;
}
char_t* Minecraft::getProgressMessage(void) { return Minecraft::progressMessages[this->progressMessageIndex]; }
int32_t Minecraft::getProgressStageStatus(void) { return this->progressMessageIndex; }
int32_t Minecraft::getProgressStatusId(void) { return this->progressMessageIndex; }
std::shared_ptr<TextureAtlas> Minecraft::getTextureAtlas(TextureAtlasId) { return 0; }
...
void Minecraft::handleBuildAction(struct BuildActionIntention* a2) {
	int field_D0C; int hitType; int entityId; int v7; int v9; int v10; int v11; int v12;
	Tile* v13; ItemInstance* v14; int v15; uint8_t v16; int v17; int v18; ItemInstance* sel; GameMode* gameMode;

	field_D0C = this->field_D0C;
	if(field_D0C > 0) this->field_D0C = field_D0C - 1;
	...
	if(!this->player->isUsingItem()) {
		...
		if(hitType == 1) {
			...
			if((a2->field_0 & 0x10) == 0 || this->field_D0C) goto LABEL_27;
...
void Minecraft::init(void) {
	...
	this->supportsNonTouchScreen = !AppPlatform::_singleton->supportsTouchscreen();
	...
	AppPlatform::_singleton->supportsTouchscreen();
...
bool_t Minecraft::isCreativeMode(void) { return this->isCreative; }
bool_t Minecraft::isKindleFire(int32_t a2) { return 0; }
bool_t Minecraft::isLevelGenerated(void) { return this->level ? this->levelGenerated : 0; }
bool_t Minecraft::isOnline(void) { return this->serverSideNetworkHandler != 0; }
bool_t Minecraft::isOnlineClient(void) { return this->level ? this->level->isClientMaybe : 0; }
...
void Minecraft::leaveGame(bool_t a2, bool_t a3) {
	...
	if(!this->field_CF4 && this->levelGenerated) {
		...
		if(v7) {
			...
			if(field_CF4) this->level->savePlayers();
		}
		this->viewEntityMaybe = 0;
		...
		this->field_CFC = 0;
		if (a3) {
			if (a2) this->setScreen(new RenameMPLevelScreen(LevelStorageSource::TempLevelId));
			else this->screenChooser.setScreen(START_MENU_SCREEN);
		}
	}
}

// PROTECCION PS3: Desactivamos el escaneo de red crudo para evitar cuelgues de socket
void Minecraft::locateMultiplayer(void) {
	this->field_18 = 0;
}

void Minecraft::lockForControl(void) { this->cmutex->lock(); }
...
void Minecraft::prepareLevel(const std::string& a2) {
	Level* level;
	int32_t v4; int32_t v5; int32_t i; int32_t v7; int32_t v8;
	LevelChunk* chunk; int32_t j; Level* v11;

	this->progressMessageIndex = 1;

	level = this->level;
	if(!level->field_B64) level->setUpdateLights(0);
	v4 = 0; v5 = 8;
	do {
		v7 = v4;
		for(i = 8; i != 264; i += 16) {
...
void Minecraft::releaseMouse(void) {
	if(this->mouseGrabbed) {
		if(this->player) this->player->releaseAllKeys();
		this->mouseGrabbed = 0;
...
void Minecraft::resetPlayer(Player* a2) {
	this->level->validateSpawn();
	a2->reset();
	TilePos result = a2->hasRespawnPosition() ? a2->getRespawnPosition() : this->level->getSharedSpawnPos();
...
void Minecraft::selectLevel(const std::string& a2, const std::string& a3, const struct LevelSettings& a4) {
	printf("[Minecraft] selectLevel: id='%s', nombre='%s'\n", a2.c_str(), a3.c_str()); fflush(stdout);

	LevelStorage* v7 = this->levelStorageSource->selectLevel(a2, 0);
	printf("[Minecraft] LevelStorage creado. Instanciando ServerLevel...\n"); fflush(stdout);

	this->level = new ServerLevel(v7, a3, a4, 0, 0);
	printf("[Minecraft] ServerLevel creado. Llamando a setLevel...\n"); fflush(stdout);

	this->setLevel(this->level, "Generating level", 0);

	if (this->level && this->level->getLevelData()) {
		this->setIsCreativeMode(this->level->getLevelData()->getGameType() == 1);
	}
	this->field_CFC = 1;
	printf("[Minecraft] selectLevel completado con exito.\n"); fflush(stdout);
}
...
void Minecraft::setIsCreativeMode(bool_t a2) {
	...
	LABEL_8:
	if(this->player) this->gameMode->initAbilities(this->player->abilities);
}

void Minecraft::setLeaveGame(void) { this->field_D22 = 1; }

void Minecraft::setLevel(struct Level* level, const std::string& a3, struct LocalPlayer* a4) {
	printf("[Minecraft] setLevel invocado...\n"); fflush(stdout);
	this->viewEntityMaybe = 0;
	if(level) {
		level->getSeed();
		...
		this->field_CF4 = 1;
		printf("[Minecraft] Creando hilo de preparacion del nivel...\n"); fflush(stdout);
		this->prepareLevelThread = new CThread(&Minecraft::prepareLevel_tspawn, this);
	} else {
		this->player = 0;
	}
	this->field_C54 = 0;
	this->field_CFC = 1;
	printf("[Minecraft] setLevel finalizado.\n"); fflush(stdout);
}
...
void Minecraft::setSize(int32_t a2, int32_t a3) {
	...
	float ppm = AppPlatform::_singleton ? AppPlatform::_singleton->getPixelsPerMillimeter() : 10;
...
bool_t Minecraft::supportNonTouchscreen(void) { return this->supportsNonTouchScreen; }

void Minecraft::teardown(void) {
	if(this->serverSideNetworkHandler) delete this->serverSideNetworkHandler;
	if(this->rakNetInstance) delete this->rakNetInstance;
	if(this->levelRenderer) delete this->levelRenderer;
	if(this->gameRenderer) delete this->gameRenderer;
	if(this->particleEngine) delete this->particleEngine;
	if(this->soundEngine) delete this->soundEngine;
	if(this->gameMode) delete this->gameMode;
	if(this->font) delete this->font;
	if(this->texturesPtr) delete this->texturesPtr;
	...
	if(this->levelStorageSource) delete this->levelStorageSource;
	if(this->perfRenderer) delete this->perfRenderer;
	if(this->commandServer) delete this->commandServer;
	if(this->cmutex) delete this->cmutex;
	if(this->field_D30) delete this->field_D30;

	MobFactory::getStaticTestMob(0, 0);
...
void Minecraft::tick(int32_t a2, int32_t a3) {
	...
	if(this->field_D08 > 0) this->field_D08--;

	if(!this->currentScreen && this->player && this->player->health <= 0) {
		this->setScreen(new DeathScreen());
	}

	if(this->level && !this->field_16C) this->gameMode->tick();
	if(this->level && this->commandServer) this->commandServer->tick();
	...
	if(!this->field_CF4) {
		...
		if(this->level && !this->field_16C) {
			...
			if(this->player) {
				...
				int v15 = (int)posX;
				if(posX < (float)v15) --v15;

				float posY = this->player->posY;
				int v18 = (int)posY;
				if(posY < (float)v18) --v18;

				float posZ = this->player->posZ;
				int v21 = (int)posZ;
				if(posZ < (float)v21) --v21;
		...
		if(this->currentScreen && !this->currentScreen->renderGameBehind()) goto DONT_TICK_GAME;
...
void Minecraft::tickInput(void) {
	...
	while(1) {
		...
		if(Keyboard::_index + 1 >= Keyboard::_inputs.size()) break;
...
void Minecraft::unlockControl(void) { this->cmutex->unlock(); }

void Minecraft::update(void) {
	static int numUpdate = 0;
	static bool guiConfigInitialized = false;

	if(!guiConfigInitialized && this->texturesPtr != nullptr) {
		this->gui.onConfigChanged(createConfig(this));
		guiConfigInitialized = true;
	}
	...
	if(this->rakNetInstance) this->rakNetInstance->runEvents(this->serverSideNetworkHandler);
	...
	if(this->level && !this->field_CF4) this->level->updateLights();

	if(this->gameMode) this->gameMode->render(this->timer.field_8);
...
void Minecraft::updateStatusUserAttributes(void) {
	const char* vendor = (const char*)glGetString(0x1F00);
	const char* renderer = (const char*)glGetString(0x1F01);
	std::string v_str = vendor ? vendor : "PS3_Vendor";
	std::string r_str = renderer ? renderer : "PS3_RSX";
	AppPlatform::_singleton->updateStatsUserData(v_str, r_str);
}
